...
1. **Identify the failed run**
   ...
   - Filter by `status: failure` and `workflow: Playwright Tests`

2. **Get the run details**
   - Call `mcp_github_get_workflow_run` with the run ID
   - Note which jobs failed (`bdd-smoke` or `bdd-regression`) and which matrix projects

3. **Download artifacts**
   - Call `mcp_github_list_workflow_run_artifacts` to list available artifacts
   - Download `playwright-traces-smoke` or `playwright-traces-{project}` using
     `mcp_github_download_workflow_run_artifact`
   - Also download `playwright-report-{job}` for the HTML report if available
...
# Download traces from a specific run
gh run download [RUN_ID] --name playwright-traces-smoke --dir tmp/remote-traces/
