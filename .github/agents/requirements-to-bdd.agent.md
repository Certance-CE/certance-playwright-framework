...
- **Frontmatter:** `epic`, `feature` (a feature tag),
  `journey` (a journey key), default `priority`.
...
- **Each acceptance criterion becomes exactly one Scenario** (the "one test,
  one scenario" golden rule).
...
3. Keep steps declarative and business-readable — follow the golden rules.
