...
**Usage**: Get synthetic test data from the `data` fixture (`fixtures/data.fixture.ts`); the factory snippets above are the illustrative pattern behind it.
...
class CreateTodoWorkflow extends Command {
  /* */
}
```
...
// Usage: new TodoPageBuilder().withFilter('completed').withSort('date').build()
```
