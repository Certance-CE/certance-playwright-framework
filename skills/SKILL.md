...
12. **Application-agnostic core** — the `skills/`, `fixtures/`, `utils/`,
    ...
    `features/`, `plans/`, and `.env` are client-specific.
...
**Review step:** Show the markdown plan to the client stakeholder before
...
├── plans/                        # Planner agent output — review with client
├── test-data/
...
│   └── [client-name]/            # Client-specific context (per engagement)
└── docs/
    ├── ARCHITECTURE.md           # Full framework architecture reference
    ├── ONBOARDING.md             # New client setup in < 2 hours
...
| Guide                                                               | Load when                                                                                     |
| ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| [Token efficiency](./token-efficiency/SKILL.md)                     | Reducing AI token / credit burn — context hygiene, model routing, output shaping, compression |
...
| [Prompt efficiency](./prompt-efficiency/SKILL.md)                   | Rewriting prompts to be file-scoped, bounded, and response-shaped                             |
| [Token measurement](./token-measurement/SKILL.md)                   | Defining before/after metrics and per-test creation cost for a token audit                    |

---

## BDD layer — Gherkin + playwright-bdd

Every Certance engagement uses a Gherkin layer on top of Playwright Test via
...
npm run bdd:test:all # run on Chromium
```
...
## `seed.spec.ts` — required for Planner agent

Authenticates, seeds test data, and saves `storageState`. Must pass reliably —
a flaky seed breaks the entire planning session. See `tests/seed.spec.ts`.
...
- Chromium only (Firefox + WebKit dropped by project decision) — re-enable cross-browser projects per client need

---

## Agent instruction files

Each AI agent reads a different conventions file. All must exist in every client repo.
...
- [ ] Clone the Certance Lens template repo
- [ ] Set `BASE_URL`, `TEST_USER_EMAIL`, `TEST_USER_PASSWORD` in `.env`
- [ ] Run `npm install` and `npx playwright install --with-deps`
- [ ] Run `npx playwright init-agents` to generate agent definitions
- [ ] Install skills: `npx skills add certance/playwright-skill`
- [ ] Write and verify `tests/seed.spec.ts` — must pass cleanly
- [ ] Create `.github/copilot-instructions.md`
- [ ] Create `.claude/CLAUDE.md`
- [ ] Create `.cursorrules`
- [ ] Run `npx playwright test tests/seed.spec.ts` — confirm green
- [ ] Run Planner agent against staging environment
- [ ] Review generated `plans/*.md` with client stakeholder
...
Semantic versioning. Pin the skill version in each client repo.
Upgrades are a deliberate, billable activity — not an automatic dependency update.

**Certance Lens** — UI Automation Framework, by Certance Advisory · Framework v1.0.0 · Playwright ^1.56.0 · Last reviewed: July 2026

<!-- Compatibility floor is ^1.56.0: Playwright Agents / `npx playwright init-agents` require ≥1.56. Latest release at review: v1.61.1 (2026-06-23). -->
