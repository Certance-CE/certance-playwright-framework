...
Load this guide when: setting up CI for a new client, modifying the pipeline,
...
Set these in the client repository under **Settings → Secrets and variables → Actions**:
...
1. Copy `.github/workflows/playwright.yml` to the client repo
2. Add secrets (see Required Secrets above)
3. Adjust the `branches` trigger to match the client's branching model
