...
A general, opinionated **Playwright + BDD** test-automation framework for web apps. It ships
with a runnable example against the public [TodoMVC demo](https://demo.playwright.dev/todomvc),
so `npm test` is green on a fresh clone — then you point `BASE_URL` at your own app and swap in
your own Page Objects.
...
| Rule                                                                      | Why it matters                                                                                                                           |
| ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Locators: `getByRole` → `getByLabel` → `getByTestId`, never CSS/XPath** | Tests bind to what a user perceives, not to brittle DOM structure. When the markup changes but behaviour doesn't, the test still passes. |
